<template><div><h1 id="网游喊话加密call分析" tabindex="-1"><a class="header-anchor" href="#网游喊话加密call分析"><span>网游喊话加密Call分析</span></a></h1>
<blockquote>
<p>思路整理</p>
</blockquote>
...
<h2 id="封包追踪" tabindex="-1"><a class="header-anchor" href="#封包追踪"><span>封包追踪</span></a></h2>
<p>观察该WSASend函数的堆栈，可知第二个参数是发送封包的包地址pBuffers，数据窗口中跟随pBuffers，可以大致看到如下内容</p>
<p><img src="https://wqby-1304194722.cos.ap-nanjing.myqcloud.com/img/image-20240212093311469.png" alt="image-20240212093311469"></p>
<p>可以推测出第一个字节16是该封包的长度，推测pBuffers+4是包数据地址。故[pBuffers]是包长，[pBuffers+4]是包数据。</p>
<p>接下来我们要对该pBuffers进行跟踪，找到线程内发包获取pBuffers的基地址。随后对其下硬件写入断点，即可找到线程外对pBuffers基地址赋值的语句，从而跳出循环。然后就可以向上找到喊话call了。</p>
<p>我们返回到调用WSASend上层，寻求倒数第二个压栈的push，定位到pBuffers来源。</p>
<p><img src="https://wqby-1304194722.cos.ap-nanjing.myqcloud.com/img/image-20240212161613045.png" alt="image-20240212161613045"></p>
<p>接下来只需要对eax进行向上追溯，就可以找到包地址来源了。</p>
<p>首先eax为esp+0x28，所以只需要追溯esp即可，可以看到包长来源是eax。继续向上追溯。</p>
<p><img src="https://wqby-1304194722.cos.ap-nanjing.myqcloud.com/img/image-20240212165126247.png" alt="image-20240212165126247"></p>
<p>可以看到包数据地址的来源是ecx，同理继续追溯ecx，ecx==edx+esi，调试可知edx恒为0，即ecx == esi，继续追溯esi，esi为[ebx+0x4]，ebx为[edx+0x2888]，综上，包数据地址==[[edi+0x2888]+0x4]</p>
<p>那么，即有其他代码向edi+0x2888写入数据之时，也就会触发线程内的发包函数。</p>
<p>我们尝试对edi+0x2888下硬件写入断点</p>
<p><img src="https://wqby-1304194722.cos.ap-nanjing.myqcloud.com/img/image-20240212171146893.png" alt="image-20240212171146893"></p>
<p>发现代码仍然在线程内，并且在刚刚代码的上方，没关系，我们继续追溯edi的来源eax，直到函数头的时候，得到[edi+2888]== [[[edi+0x2880]]+0x8] ，测试得知edi+0x2880的值是不变的，故继续对[edi+0x2880]下硬件写入断点（此时edx==[edi+0x2880]为固定值）</p>
<p><img src="https://wqby-1304194722.cos.ap-nanjing.myqcloud.com/img/image-20240212215611858.png" alt="image-20240212215611858"></p>
<p>此时断到的位置往上堆栈回溯会发现与WSASend的上层堆栈就不一样了，至此为止说明我们已经跳出了线程，找到了线程外对线程内发包地址进行赋值的代码。</p>
<p>至此，包数据地址的表达式为[[[[edi+0x2880]]+0x8]+0x4]，并且[edi+0x2880]是固定值</p>
<p>对[eax]赋值ebp，也就是对前面的[edi+0x2880]赋值ebp，故表达式可以换成[[ebp+0x8]+0x4]</p>
<h2 id="加密追踪" tabindex="-1"><a class="header-anchor" href="#加密追踪"><span>加密追踪</span></a></h2>
<p>可以揣测此时[[ebp+0x8]+0x4]就是加密后的封包数据，我们跳到上层CALL下断继续分析参数。</p>
<p><img src="https://wqby-1304194722.cos.ap-nanjing.myqcloud.com/img/image-20240216020431985.png" alt="image-20240216020431985"></p>
<p>追踪[ESI+4]</p>
<p><img src="https://wqby-1304194722.cos.ap-nanjing.myqcloud.com/img/image-20240216020450258.png" alt="image-20240216020450258"></p>
<p>发现可以看到了封包的明文喊话内容11111111111的字节形式，而第一个字节21便是包长度，于是我们可以猜测加密明文封包内容的代码就在这个CALL中，封包内容明文的表达式便是[ESI+4]我们进入CALL中继续分析。</p>
<p><img src="https://wqby-1304194722.cos.ap-nanjing.myqcloud.com/img/image-20240217121734101.png" alt=""></p>
<p>前面五个判断都直接被跳过了，所以应该是判断一些封包内容合法之类的无用操作，可以不用管。</p>
<p>那么封包加密操作就在下面的三个CALL中了。</p>
<p>通过观察[esi+4]的值在执行call前后是否产生变化，找到加密CALL。</p>
<p>在调用加密CALL之前，通过调试可以猜测出eax是类似于加密秘钥的一个参数，我们要先追溯出eax的来源。</p>
<p>这里不做过多赘述，最后可以得到eax==[[[[[00f84ba4]]+4]+0xC+8]]+0x54</p>
<h2 id="调用加密call" tabindex="-1"><a class="header-anchor" href="#调用加密call"><span>调用加密call</span></a></h2>
<p>首先观察call内容，发现除了esp以外，没有用到任何一个寄存器的值。所以如果只是单纯调用该加密call，只需要一个正确的esp值即可。</p>
<p><img src="https://wqby-1304194722.cos.ap-nanjing.myqcloud.com/img/image-20240217131642975.png" alt="image-20240217131642975"></p>
<p>编写测试加密代码</p>
<p><img src="https://wqby-1304194722.cos.ap-nanjing.myqcloud.com/img/image-20240217170720453.png" alt="image-20240217170720453"></p>
<p>复制一份执行加密call时ebp和ebx的值到01111111里，然后注入代码，测试出加密成功</p>
<h2 id="编写代码发送封包" tabindex="-1"><a class="header-anchor" href="#编写代码发送封包"><span>编写代码发送封包</span></a></h2>
<p>我们尝试手写一个MFC的dll，直接将dll注入至游戏内部，通过调用汇编代码实现加密功能，再调用windows Api的WSASend实现模拟封包发送。</p>
<p>先编写一个可视化的窗口</p>
<p><img src="https://wqby-1304194722.cos.ap-nanjing.myqcloud.com/img/image-20240217174249635.png" alt="image-20240217174249635"></p>
<p>接着编写封包发送，首先我们要逆向一下WSASend的第一个参数socket的来源</p>
<p><img src="https://wqby-1304194722.cos.ap-nanjing.myqcloud.com/img/image-20240218210422616.png" alt="image-20240218210422616"></p>
<p>追踪ecx，不断往上层去返回</p>
<p><img src="https://wqby-1304194722.cos.ap-nanjing.myqcloud.com/img/image-20240218211214918.png" alt="image-20240218211214918"></p>
<p>最后可以得到socket的值是GetWindowLongW()的返回值加上0x38地址的值，而GetWindowLongW的第一个参数是根据类名Lapis Network Class得到的窗口句柄，第二个参数为固定值GWL_USERDATA。</p>
<p>故综上可以编写出如下代码</p>
<p><img src="https://wqby-1304194722.cos.ap-nanjing.myqcloud.com/img/image-20240218211447461.png" alt="image-20240218211447461"></p>
<p>将该程序DLL注入到游戏即可实现模拟发送喊话封包了</p>
<p><img src="https://wqby-1304194722.cos.ap-nanjing.myqcloud.com/img/image-20240221205530581.png" alt="image-20240221205530581"></p>
<h2 id="纯汇编实现加密" tabindex="-1"><a class="header-anchor" href="#纯汇编实现加密"><span>纯汇编实现加密</span></a></h2>
<blockquote>
<p>这样的话我们虽然实现了功能，但是加密部分仍然是调用了游戏的CALL，我们并没有实现完全依靠自己的代码模拟实现，所以要继续深究</p>
</blockquote>
<p>将加密CALL的汇编代码抠出来，做成如下函数</p>
<div class="language-c line-numbers-mode" data-ext="c" data-title="c"><pre v-pre class="language-c"><code><span class="token function">__declspec</span><span class="token punctuation">(</span>naked<span class="token punctuation">)</span> <span class="token keyword">void</span> 加密<span class="token function">CALL</span><span class="token punctuation">(</span>DWORD 秘钥<span class="token punctuation">,</span>DWORD 加密长度<span class="token punctuation">,</span>DWORD 加密地址<span class="token punctuation">,</span>DWORD 加密地址<span class="token number">2</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
	__asm
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>至此就可以实现完全脱离游戏本身代码实现模拟封包的加密与发送了。</p>
<h2 id="编写简易抓包工具" tabindex="-1"><a class="header-anchor" href="#编写简易抓包工具"><span>编写简易抓包工具</span></a></h2>
<p>除了喊话封包以外，我们后面肯定还要模拟其他功能的封包实现。所以我们要对明文封包进行HOOK实现自动抓取。</p>
<div class="language-c line-numbers-mode" data-ext="c" data-title="c"><pre v-pre class="language-c"><code><span class="token keyword">void</span> <span class="token function">outputMsg</span><span class="token punctuation">(</span><span class="token keyword">char</span><span class="token operator">*</span> pszFormat<span class="token punctuation">,</span> <span class="token punctuation">.</span><span class="token punctuation">.</span><span class="token punctuation">.</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
...
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><img src="https://wqby-1304194722.cos.ap-nanjing.myqcloud.com/img/image-20240223033140392.png" alt=""></p>
<blockquote>
<p>结语</p>
</blockquote>
<p>前不久找到了一个满眼都是自己的女孩，沉浸在爱情的海洋里，3个月没碰代码。过完年也算是闲了下来，准备继续深造技术。去年的项目都停了，今年从0开始全新的领域，目标20个W，希望今年能用编程赚到的钱开起来一个属于自己的台球厅。</p>
</div></template>
