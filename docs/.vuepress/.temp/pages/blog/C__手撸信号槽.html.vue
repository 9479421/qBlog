<template><div><h1 id="c-手撸信号槽" tabindex="-1"><a class="header-anchor" href="#c-手撸信号槽"><span>C++手撸信号槽</span></a></h1>
...
<p>将发送者定义为接收者的类成员，并在发送者构造函数中，绑定发送者的send信号和接收者的recv函数。绑定成功后，意味着只要send信号被发送，recv函数就会被自动执行。</p>
...
<p>接下来我们来设计这个Signal信号类，首先信号类要存储一个或多个对应的接收者对象和接收者成员函数以及函数的参数，因为一个信号可以绑定多个触发对象和函数，所以我们想到用std::vector来存储这些内容。</p>
...
<p>发现可以实现了多个接收者与一个发送者进行了函数绑定，当发送者的信号发出时，两个不同的接收者都执行了对应的函数回调。</p>
